...
[ -f "${srcdir}/bin/cother" ] || (echo "ERROR: Incomplete software package: executable cother missing!" && exit 1)
...
cmd="${srcdir}/bin/cother --dev-list >/dev/null 2>&1"
eval ${cmd} || cat <<EOF

WARNING: The cother executable will not run on the system!
