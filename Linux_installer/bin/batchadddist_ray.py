...
description = "Include pairwise distance information in each profile "+ \
    "present in the given directory. (Ray version to parallelization)"
proext = '.pro'
tproext = '.tpro'
...
@ray.remote
def processProfile(index, nprofiles, step, profiles_id, options):
    """Process one profile by adding distance information to it and 
    writing the resulting cother profile to file. 
    """
    for i in range(index, nprofiles, step):
        profilename = profiles_id[i]
...
if __name__ == "__main__":
    ...
    profiles_id = ray.put(profiles)
    dummy = [processProfile.remote(i, len(profiles), n_cores, profiles_id, options) for i in range(n_cores)]
    ray.get(dummy)
