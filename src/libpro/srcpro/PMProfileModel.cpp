...
const char* PMProfileModel::dataversionCOMER = "2.0";
// COTHER profile format version
const char* PMProfileModel::dataversion = "2.5";
const char* PMProfileModel::bindataversion = "2.5";
...
/*static */const char* patstrDATVER = "COTHER profile v";
/*static */const char* patstrDATVERCOMER = "COMER profile v";
...
// ndstvals, total number of distance values over all positions
//
void CalculateAndWriteAddressTable_v2_2(
    ...
    const size_t nprofiles, const size_t totalposs, const size_t /*ndstvals*/)
{
    if( addrfields == NULL )
    ...
    addr += totalposs * sizeof(float);//end
    //{{
    //NOTE: the first cell of the distances at each profile position is 
    // reserved for the number of distance values (variable); this makes efficient 
    // memory access on GPU
    for( t = 0; t < ptr2DNoDstsPerPos; t++ ) {
        table[n++] = addrfields[pmv2DDDvalues+t] = addr;//block of IR distance values (for each t)
        addr += totalposs * sizeof(float);//end
    }
    //}}
    ...
    const PMProfileModel& pssm, const PMTransModel& gaps )
{
    ...
    //{{block of inter-residue distances
    const size_t nmaxdstvals = MAX_N_IR_DISTANCE_VALUES;
    //const size_t nmaxdstvalsp1 = MAX_N_IR_DISTANCE_VALUES_PLUS_LENGTH;
    const static float optdstgapl = 255.f;//(0xff)//MOptions::GetDSTGAPL();
    float dstval;
    //write the number of entries
    //{
        spvals.Clear();
        bytes = 0;
        if( MAXFILESIZE <= addrfields[pmv2DDDvalues])
            throw MYRUNTIME_ERROR("BinaryWriteProfile_v2_2: Invalid address of IR distances.");
        fp.seekp(std::streamoff(addrfields[pmv2DDDvalues]));
        if(fp.fail())
            throw MYRUNTIME_ERROR("BinaryWriteProfile_v2_2: Failed to set file position.");
        for( m = 0; m < pssm.GetSize(); m++ ) {
            res = pssm.GetResidueAt(m);
            ...
            dstval = 0.0f;//length
            if(!(Xuninf && res == X))
                dstval = (float)(pssm.GetDistancesAt(m).size()<nmaxdstvals? 
                    pssm.GetDistancesAt(m).size(): nmaxdstvals);
            spvals.Push(dstval);
            bytes += sizeof(dstval);
        }
        fp.write(reinterpret_cast<const char*>(spvals.GetVector()),bytes);
        if(fp.bad())
            throw MYRUNTIME_ERROR("BinaryWriteProfile_v2_2: Write to file failed.");
        addrfields[pmv2DDDvalues] += bytes;
    //}
    //write the data
    for( t = 0; t < (int)nmaxdstvals; t++ ) {
        spvals.Clear();
        bytes = 0;
        if( MAXFILESIZE <= addrfields[pmv2DDDvalues+1+t])
            throw MYRUNTIME_ERROR("BinaryWriteProfile_v2_2: Invalid address of IR distances.");
        fp.seekp(std::streamoff(addrfields[pmv2DDDvalues+1+t]));
        if(fp.fail())
            throw MYRUNTIME_ERROR("BinaryWriteProfile_v2_2: Failed to set file position.");
        for( m = 0; m < pssm.GetSize(); m++ ) {
            res = pssm.GetResidueAt(m);
            if( res == GAP )
                continue;//omit unused positions and gaps
            dstval = optdstgapl;
            if(!(Xuninf && res == X) && t < (int)pssm.GetDistancesAt(m).size())
                dstval = pssm.GetDistancesAt(m)[t];
            spvals.Push(dstval);
            bytes += sizeof(dstval);
        }
        fp.write(reinterpret_cast<const char*>(spvals.GetVector()),bytes);
        if(fp.bad())
            throw MYRUNTIME_ERROR("BinaryWriteProfile_v2_2: Write to file failed.");
        addrfields[pmv2DDDvalues+1+t] += bytes;
    }
    //}}//distances
...
//         fprintf( fp, "%s", NL);
        ...
        if(m < (int)pssm.GetDistogram().size() && pssm.GetDistancesAt(m).size()) {
            fprintf( fp, "%s %s %d", NL, patstrD, (int)pssm.GetDistancesAt(m).size());
            std::for_each(pssm.GetDistancesAt(m).begin(), pssm.GetDistancesAt(m).end(), 
                [fp/*,scale*/](const float& d){fprintf(fp," %d",(int)rintf(/*scale**/d));});
        }
        //}}
    }
...
// =========================================================================
// TextWriteProfileCondensedCOMER: write condensed COMER profile data to 
// file
//
void TextWriteProfileCondensedCOMER( 
    FILE* fp,
    const PMProfileModel& pssm, const PMTransModel& gaps,
    int scale )
{
    ...
    if( pssm.GetSize() != gaps.GetSize())
        throw MYRUNTIME_ERROR("TextWriteProfileCondensedCOMER: Inconsistent Profile data.");

    if( scale < 1 )
        throw MYRUNTIME_ERROR("TextWriteProfileCondensedCOMER: Invalid scale factor.");
    ...
    fprintf( fp, "%s%s%s", patstrDATVERCOMER, PMProfileModel::dataversionCOMER, NL );
    fprintf( fp, "%s %s%s", patstrDESC, pssm.GetDescription()? pssm.GetDescription(): "", NL );
    fprintf( fp, "%s %s%s", patstrFILE, pssm.GetName()? pssm.GetName(): "", NL );
...
//         fprintf( fp, "%s", NL);
        ...
        if( 0 < (int)noppps ) {
            if( ppprobs == NULL || pppndxs == NULL )
                throw MYRUNTIME_ERROR("TextWriteProfileCondensedCOMER: Null posterior probabilities.");
        ...
        if( pssm.GetctPsSet()) {
            ...
            if( 0 < (int)noppps ) {
                if( ppprobs == NULL || pppndxs == NULL )
                    throw MYRUNTIME_ERROR("TextWriteProfileCondensedCOMER: Null ct posterior probabilities.");
        ...
        if( pssm.GetCtxVecSet()) {
            ...
            if( 0 < vsize ) {
                if( ctxvec == NULL )
                    throw MYRUNTIME_ERROR("TextWriteProfileCondensedCOMER: Null context vector.");
...
// -------------------------------------------------------------------------
// TextReadProfileCOMER: read COMER profile data from file
// TODO: reimplement large arrays to use heap (locbuffer)
//
void TextReadProfileCOMER( FILE* fp, PMProfileModel& pssm, PMTransModel& gaps )
{
    if( !fp )
        return;

    const mystring preamb = "TextReadProfileCOMER: ";
...
// //     int         delint;
    ...
    if(( p = strstr( locbuffer, patstrDATVERCOMER )) == NULL )
        throw MYRUNTIME_ERROR( preamb + "Wrong profile format." );

    p += strlen( patstrDATVERCOMER );

    if( length <= size_t( p - locbuffer ))
        throw MYRUNTIME_ERROR( preamb + "Wrong profile format." );

    if(( p = strstr( p, PMProfileModel::dataversionCOMER )) == NULL )
...
// -------------------------------------------------------------------------
// TextReadProfile: read (COTHER) profile data from file
//
void TextReadProfile(
    FILE* fp, PMProfileModel& pssm, PMTransModel& gaps)
{
    if( !fp )
        return;
    ...
    bool            lineread = false;
    const size_t    locsize = 10*KBYTE;
    std::unique_ptr<char[]> locbuffer(new char[locsize+1]);
    std::vector<float> vtmpdsts(optdstnmax);//temporary vector of distances
    char            msgbuf[BUF_MAX];
    char*           p;
    int             emsg;
    mystring    desc, file;
...
// //     int         delint;
    ...
    //read version number
    if(( emsg = skip_comments(fp, locbuffer.get(), locsize, &length )) != 0)
        throw MYRUNTIME_ERROR( preamb + TranslateReadError( emsg ));

    if( feof( fp ) || !length )
        throw MYRUNTIME_ERROR( preamb + "Wrong profile format." );

    if(( p = strstr(locbuffer.get(), patstrDATVER)) == NULL )
        throw MYRUNTIME_ERROR( preamb + "Wrong profile format." );

    p += strlen( patstrDATVER );

    if( length <= size_t( p - locbuffer.get() ))
        throw MYRUNTIME_ERROR( preamb + "Wrong profile format." );
    ...
    if(( p = strstr(locbuffer.get(), patstrCMD)) == NULL )
        throw MYRUNTIME_ERROR( preamb + "Wrong profile format." );
    ...
    if(( p = strstr(locbuffer.get(), patstrLEN)) == NULL )
        throw MYRUNTIME_ERROR( preamb + "Wrong profile format." );
    ...
    if(( emsg = read_integer(p, length - size_t(p-locbuffer.get()), &prolen, &rbts )) != 0)
        throw MYRUNTIME_ERROR( preamb + TranslateReadError( emsg ));
    ...
    if(( p = strstr(locbuffer.get(), patstrNOS)) == NULL )
        throw MYRUNTIME_ERROR( preamb + "Wrong profile format." );
    ...
    if(( emsg = read_integer(p, length - size_t(p-locbuffer.get()), &nos, &rbts)) != 0 )
        throw MYRUNTIME_ERROR( preamb + TranslateReadError( emsg ));
    ...
    if(( p = strstr(locbuffer.get(), patstrEFFNOS)) == NULL )
        throw MYRUNTIME_ERROR( preamb + "Wrong profile format." );
    ...
    if(( emsg = read_float(p, length - size_t(p-locbuffer.get()), &effnos, &rbts )) != 0)
        throw MYRUNTIME_ERROR( preamb + TranslateReadError( emsg ));
    ...
    if(( p = strstr(locbuffer.get(), patstrSCALE)) == NULL )
        throw MYRUNTIME_ERROR( preamb + "Wrong profile format." );
    ...
    if(( emsg = read_integer(p, length - size_t(p-locbuffer.get()), &scale, &rbts )) != 0)
        throw MYRUNTIME_ERROR( preamb + TranslateReadError( emsg ));
    ...
    //read background probabilities
    if(( emsg = skip_comments(fp, p = locbuffer.get(), locsize, &length)) != 0 )
        throw MYRUNTIME_ERROR( preamb + TranslateReadError( emsg ));

    if( feof( fp ) || !length )
        throw MYRUNTIME_ERROR( preamb + "Wrong profile format." );

    if((p = strstr(locbuffer.get(), patstrNULL)) == NULL)
        throw MYRUNTIME_ERROR( preamb + "Wrong profile format: No background probabilities." );
    ...
    for( r = 0; r < noress; r++ ) {
        ...
        if((emsg = read_integer( p, length - size_t(p-locbuffer.get()), &intval, &rbts )) != 0)
            throw MYRUNTIME_ERROR( preamb + TranslateReadError( emsg ));
    ...
    if(( p = strstr(locbuffer.get(), patstrPOST)) == NULL )
        throw MYRUNTIME_ERROR( preamb + "Wrong profile format: No posterior probabilities." );
    ...
    for( m = 0; m < prolen; m++ )
    {
        ...
        if(( emsg = read_integer(p = locbuffer.get(), length, &n, &rbts)) != 0 ) {
            sprintf( msgbuf, "Wrong profile format at pos %d: ", m );
            throw MYRUNTIME_ERROR( preamb + msgbuf + TranslateReadError( emsg ));
        }
        ...
        if(( emsg = read_symbol(p, length - size_t(p-locbuffer.get()), &res, &rbts)) != 0 ) {
            sprintf( msgbuf, "Wrong profile format at pos %d: ", m );
            throw MYRUNTIME_ERROR( preamb + msgbuf + TranslateReadError( emsg ));
        }
        ...
        //{{weight and information skipped}}
        ...
        if( pssm.GetctPsSet()) {
            ...
            if(( p = strstr(locbuffer.get(), patstrCT)) == NULL ) {
                if( m < 1 )
                    pssm.SetctPsSet( false );//no HDP ctx information
                else {
                    sprintf( msgbuf, "Wrong profile format at pos %d: "
        ...
        if( pssm.GetCtxVecSet()) {
            ...
            if(( p = strstr(locbuffer.get(), patstrCV)) == NULL ) {
                if( m < 1 )
                    pssm.SetCtxVecSet( false );//no context vector data
                else {
                    sprintf( msgbuf, "Wrong profile format at pos %d: "
        ...
        if( pssm.GetSSSSet()) {
            ...
            if(( p = strstr(locbuffer.get(), patstrSS)) == NULL ) {
                if( m < 1 )
                    pssm.SetSSSSet( false );//no SS information
                else {
                    sprintf( msgbuf, "Wrong profile format at pos %d: No SS state.", m );
                ...
                if(( emsg = read_symbol(p, length - size_t(p-locbuffer.get()), &sss, &rbts)) != 0 ) {
                    sprintf( msgbuf, "Wrong profile format at pos %d: ", m );
                    throw MYRUNTIME_ERROR( preamb + msgbuf + TranslateReadError( emsg ));
                }
        ...
        if(( p = strstr(locbuffer.get(), patstrD)) != NULL ) {
            lineread = false;
            p += strlen( patstrD );

            if( length <= size_t( p - locbuffer.get() )) {
                sprintf( msgbuf, "Wrong profile format at pos %d: "
                                    "No number of distances.", m );
                throw MYRUNTIME_ERROR( preamb + msgbuf );
            }
            ...
            if( intval < 0 ) {
                sprintf( msgbuf, "Wrong profile format at pos %d: "
                                    "Negative number of distances.", m );
                throw MYRUNTIME_ERROR( preamb + msgbuf );
            }
            if( 1000 < intval ) {
                sprintf( msgbuf, "Wrong profile format at pos %d: "
                                    "Too large number of distances.", m );
                throw MYRUNTIME_ERROR( preamb + msgbuf );
            }
            ...
            for( t = 0; t < ndsts; t++ )
            {
                ...
                vtmpdsts.push_back((float)intval);
            }
        }//if patstrD
        //}}DD
...
// //         frqs.PushAt( freqns, res, m );
        ...
        if( pssm.GetCtxVecSet()) pssm.SetCtxVecPlusAt( m, norm2, lpprb, ctxvec.GetVector(), vsize );
        if( pssm.GetSSSSet()) pssm.SetSSStateAt( m, sss, sssprob );
        pssm.SetDistancesAt(m, vtmpdsts);
// //         gaps.PushAt(&trnpro, gapwgt, delbeg, delend, delint, res, m );
        gaps.PushAt(&trnpro, m );
    }
...
// #ifdef SCALE_PROFILES
    ...
    if(( p = strstr(locbuffer.get(), patstrEXPNN)) != NULL ) {
        p += strlen(patstrEXPNN);
        ...
        if(( emsg = read_float(p, length - size_t(p-locbuffer.get()), &proexpected, &rbts)) != 0 )
            throw MYRUNTIME_ERROR( preamb + 
            "Wrong profile format at the end: " + TranslateReadError( emsg ));
        ...
        if(( p = strstr(locbuffer.get(), patstrSPCOMP)) == NULL )
            throw MYRUNTIME_ERROR( preamb + 
            "Wrong profile format at the end: No statistical parameters." );
        ...
        if(( emsg = read_float(p, length - size_t(p-locbuffer.get()), &cmpunK, &rbts)) != 0 )
            throw MYRUNTIME_ERROR( preamb + 
            "Wrong profile format at the end: " + TranslateReadError( emsg ));
...
//             "Wrong profile format at the end: Invalid statistical parameters." );
        ...
        if(( emsg = read_float(p, length - size_t(p-locbuffer.get()), &cmpunlambda, &rbts)) != 0 )
            throw MYRUNTIME_ERROR( preamb + 
            "Wrong profile format at the end: " + TranslateReadError( emsg ));
...
//             "Wrong profile format at the end: Invalid statistical parameter lambda." );
        ...
        if(( p = strstr(locbuffer.get(), patstrSPREFR)) == NULL )
            throw MYRUNTIME_ERROR( preamb + 
            "Wrong profile format at the end: No reference parameters." );
        ...
        if(( emsg = read_float(p, length - size_t(p-locbuffer.get()), &refunK, &rbts)) != 0 )
            throw MYRUNTIME_ERROR( preamb + 
            "Wrong profile format at the end: " + TranslateReadError( emsg ));
        ...
        if(( emsg = read_float(p, length - size_t(p-locbuffer.get()), &refunlambda, &rbts)) != 0 )
            throw MYRUNTIME_ERROR( preamb + 
            "Wrong profile format at the end: " + TranslateReadError( emsg ));
        ...
        if(( p = strstr(locbuffer.get(), patstrENTROPY)) == NULL )
            throw MYRUNTIME_ERROR( preamb + 
            "Wrong profile format at the end: No entropy." );
        ...
        if(( emsg = read_float(p, length - size_t(p-locbuffer.get()), &proentropy, &rbts)) != 0 )
            throw MYRUNTIME_ERROR( preamb + 
            "Wrong profile format at the end: " + TranslateReadError( emsg ));
...
//             "Wrong profile format at the end: Invalid entropy." );
        ...
        if(( p = strstr(locbuffer.get(), patstrEXPECTED)) == NULL )
            throw MYRUNTIME_ERROR( preamb + 
            "Wrong profile format at the end: No expected value." );
