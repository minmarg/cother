...
public:
    enum {v2_2_NSECTIONS = 1/*pps2DLen*/ + 1/*pps2DENO*/ + NUMAA/*pps2DBkgPrbs*/ +
        ...
        MAX_N_IR_DISTANCE_VALUES_PLUS_LENGTH/*DST:pmv2DDDvalues*/ +
    ...
    static const char* dataversionCOMER;
    ...
    //{{
    const std::vector<std::vector<float>>& GetDistogram() const;
    void SetDistogram(std::vector<std::vector<float>>&&);
    const std::vector<float>& GetDistancesAt(int n) const;
    void SetDistancesAt(int n, const std::vector<float>&);
    size_t GetTotalNoDistances() const;
    //}}
...
// //     float     (*trgfrqs_)[NUMAA];       //target frequencies
    ...
    std::vector<std::vector<float>>     distogram_;//distance distribution at each position
...
void TextWriteProfileCondensed(
    FILE*, const PMProfileModel&, const PMTransModel&, int scale = PMProfileModel::ReadWriteScale);
void TextWriteProfileCondensedCOMER(
    FILE*, const PMProfileModel&, const PMTransModel&, int scale = PMProfileModel::ReadWriteScale);
void TextReadProfileCOMER(FILE*, PMProfileModel&, PMTransModel&);
...
// =========================================================================
// GetDistogram: get the full distogram
//
inline
const std::vector<std::vector<float>>& PMProfileModel::GetDistogram() const
{
    return distogram_;
}

// SetDistogram: set distogram
//
inline
void PMProfileModel::SetDistogram(std::vector<std::vector<float>>&& dstgram)
{
#ifdef __DEBUG__
    if(GetSize() != (int)dstgram.size())
        throw MYRUNTIME_ERROR("PMProfileModel::SetDistogram: Inconsistent distogram.");
#endif
    distogram_ = std::move(dstgram);
}

// GetDistancesAt: get the distances at position n
//
inline
const std::vector<float>& PMProfileModel::GetDistancesAt(int n) const
{
#ifdef __DEBUG__
    if(n < 0 || GetSize() <= n || (int)distogram_.size() <= n)
        throw MYRUNTIME_ERROR("PMProfileModel::GetDistancesAt: Memory access error.");
#endif
    return distogram_[n];
}

// SetDistancesAt: set distances at position n
//
inline
void PMProfileModel::SetDistancesAt(int n, const std::vector<float>& vdsts)
{
#ifdef __DEBUG__
    if(n < 0 || GetSize() <= n || (int)distogram_.size() <= n)
        throw MYRUNTIME_ERROR("PMProfileModel::SetDistancesAt: Memory access error.");
#endif
    distogram_[n] = vdsts;
}

// GetTotalNoDistances: calculate the total number of distances written in the profile
//
inline
size_t PMProfileModel::GetTotalNoDistances() const
{
    size_t nt = 0;
    for(const std::vector<float>& dn: distogram_)
        nt += dn.size();
    return nt;
}
