...
public:
    ...
    uint64_mt       GetNoDstValues() const      { return no_dstvalues_; }
...
//     void    WriteFrequencies( FILE* fd );//write frequency vectors to file descriptor
    ...
    void    SetNoDstValues( uint64_mt value ) { no_dstvalues_ = value; }
    void    IncNoDstValues( size_t count ) { no_dstvalues_ += count; }

    void    WriteBinaryHeader(std::ofstream&);
    void    WriteTextHeader(FILE*);
    void    ReadTextHeader(FILE*, size_t* nseqs = NULL, uint64_mt* dbsize = NULL, uint64_mt* ndstvals = NULL);
...
private:
    ...
    uint64_mt           no_dstvalues_;      //number of distance values over all positions in the database
...
public:
    ...
    static const char*  patstrNDISTANCES;
