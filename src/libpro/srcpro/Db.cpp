...
void Db::MakeBin_v2_2()
{
    ...
    try {
        ...
        pmodel::CalculateAndWriteAddressTable_v2_2(
            ...
            no_sequences_/*nprofiles*/, db_size_/*totalposs*/, no_dstvalues_);
...
void Db::ProcessFile( const char* filename, void* )
{
    ...
    try {
        ...
        try {
            ReadTextHeader( fp, &nseqs, &dbsize, &ndstvals );
        ...
        for( ;!feof(fp); )
        {
            ...
            IncNoDstValues( pm.GetTotalNoDistances());
...
const char* Db::patstrDBBINVER = "COTHER profile bin database v";
const char* Db::patstrDBVER = "COTHER profile database v";
const char* Db::patstrNoPROFS = "PROFS:";
const char* Db::patstrSIZE = "SIZE:";
const char* Db::patstrNDISTANCES = "NDISTANCES:";
...
void Db::WriteBinaryHeader( std::ofstream& fp )
{
    ...
    fp.write(reinterpret_cast<const char*>(&no_dstvalues_),sizeof(no_dstvalues_));
}
...
void Db::WriteTextHeader( FILE* fp )
{
    ...
    fprintf( fp, "%s %zu%s", patstrNoPROFS, GetNoSequences(), NL );
    fprintf( fp, "%s %llu%s", patstrSIZE, GetDbSize(), NL );
    fprintf( fp, "%s %llu%s", patstrNDISTANCES, GetNoDstValues(), NL );
}

// -------------------------------------------------------------------------
// ReadTextHeader: read database header from file descriptor
//
void Db::ReadTextHeader( FILE* fp, size_t* nseqs, uint64_mt* dbsize, uint64_mt* ndstvals )
{
    ...
    if(( p = strstr( locbuffer.get(), patstrDBVER )) == NULL )
        throw MYRUNTIME_ERROR( preamb + "Wrong database format." );
    ...
    if(( p = strstr( locbuffer.get(), patstrNoPROFS )) == NULL )
        throw MYRUNTIME_ERROR( preamb + "Wrong database format." );
    ...
    if(( emsg = read_llinteger( p, length - size_t(p-locbuffer.get()), &llintval, &rbts )) != 0 )
        throw MYRUNTIME_ERROR( preamb + TranslateReadError( emsg ));
    ...
    if(( p = strstr( locbuffer.get(), patstrSIZE )) == NULL )
        throw MYRUNTIME_ERROR( preamb + "Wrong database format." );
    ...
    //read number of distance values
    if(( emsg = skip_comments( fp, locbuffer.get(), locsize, &length )) != 0 )
        throw MYRUNTIME_ERROR( preamb + TranslateReadError( emsg ));
    ...
    if(( p = strstr( locbuffer.get(), patstrNDISTANCES )) == NULL )
        throw MYRUNTIME_ERROR( preamb + "Wrong database format." );

    p += strlen( patstrNDISTANCES );
    ...
    if( llintval < 0 )
        throw MYRUNTIME_ERROR( preamb + "Invalid number of distance values in the database." );

    if( ndstvals == NULL )
        SetNoDstValues( llintval );
    else
        *ndstvals = (uint64_mt)llintval;
}
