...
if not exist "%INSTCOTHER%" (echo ERROR: Directory does not exist: "%INSTCOTHER%" & exit /b)

set makepro=%INSTCOTHER%bin\makepro.exe
set ssp=%INSTCOTHER%bin\ssp2.pl
set inssp=%INSTCOTHER%bin\inssp2.pl
set SSFILE=%OUTPUT%.ss

if not exist "%makepro%" (echo ERROR: Executable `makepro' from the package not found: "%makepro%" & exit /b)
if not exist "%ssp%" (echo ERROR: A perl script from the package not found: "%ssp%" & exit /b)
if not exist "%inssp%" (echo ERROR: A perl script from the package not found: "%inssp%" & exit /b)
...
set cmd=perl "%ssp%" --in %INPUT% --out %SSFILE% %OPTIONS% %NOSELECT% %NOFLUSH% ^
    --psipred %INSTPSIPRED% --blast %INSTBLASTP% --this "%INSTCOTHER%"
if not "%VERBOSE%"=="" (echo %cmd%)
%cmd%  ||  exit /b
