...
void TdDataReader::CalculateTotalDbSize()
{
    ...
    for(size_t i = 0; i < dbnamelist_.size(); i++) {
        ...
        size_t szdbattr = szdbverstr + dzdbver + sizeof(size_t) + sizeof(uint64_mt) + sizeof(uint64_mt);
...
void TdDataReader::OpenAndReadPreamble(const std::string& dbname)
{
    ...
    const int maxndstvalsp1 = ptr2DNoDstsPerPos;//max. #IR distance values including the length
    int r, t;
    char* p = NULL;
    ...
    //database size
    uint64_mt dbsize = *(uint64_mt*)p;
    p += sizeof(uint64_mt);
    if( dbsize < 1 )
        throw MYRUNTIME_ERROR( preamb + "Invalid database size read.");

    //total number of IR distance values
    //uint64_mt totdsts = *(uint64_mt*)p;
    p += sizeof(uint64_mt);
    //if( totdsts < 1 )
    //    throw MYRUNTIME_ERROR( preamb + "Invalid total number of IR distances read.");
    ...
    for( t = 0; t < maxndstvalsp1; t++ )
        GetNextValue( addrtable_ + pmv2DDDvalues+t, p, filesize );//pos.-specific IR distance values (for each t)
    ...
    SbdbCData* pbdbC, size_t lengthsndx, size_t npros, size_t totlen)
{
    ...
    //READ//IR distance values for each n//[pmv2DDDvalues]
    for( n = 0; n < ptr2DNoDstsPerPos; n++ ) {
        bdbCpmbeg[pmv2DDDvalues+n] = pbdbCdata;//
        szdat = SZFPTYPE * totlen;
        dbobj_->ReadData(
            addrtable_[pmv2DDDvalues+n] + SZFPTYPE * bdbCdata_poss_from_,
            pbdbCdata, szdat );
        szval = ALIGN_UP(szdat, TdDataReader::s_szdatalign_);
        bdbCpmend[pmv2DDDvalues+n] = pbdbCdata + szdat;//
        pbdbCdata += szval;
        szpm2dvfields[pmv2DDDvalues+n+1] = szpm2dvfields[pmv2DDDvalues+n] + szval;//
    }
