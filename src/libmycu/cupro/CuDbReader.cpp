...
void CuDbReader::ReadTextHeader( TCharStream* fp )
{
    ...
    if(( p = strstr( buffer.c_str(), Db::patstrNDISTANCES )) == NULL )
        throw MYRUNTIME_ERROR( preamb + "Wrong database format." );

    p += strlen( Db::patstrNDISTANCES );
...
void CuDbReader::ReadBinaryHeader( TCharStream* fp )
{
    ...
    //read total number of IR distance values
    len = sizeof(uint64_mt);
    no_dstvalues_ = *(uint64_mt*)p;
    p += len;
    fp->incpos(len);
    if( fp->eof())
        throw MYRUNTIME_ERROR( preamb + "Wrong database format.");
    if( MAXFILESIZE < no_dstvalues_ )
        throw MYRUNTIME_ERROR( preamb + "Total number of distance values exceeds max file size.");
}
