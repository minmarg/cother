    ...
    char** pmdata )
{
    ...
    const int nmaxdstvals = MAX_N_IR_DISTANCE_VALUES;
    const int nmaxdstvalsp1 = MAX_N_IR_DISTANCE_VALUES_PLUS_LENGTH;
    ...
    extspsl::Pslvector vtmpdsts;//temporary vector of distances
...
//             pm2dvec.SetFieldNext( ptr2DTrnPrbsExp + tt, 0, (FPTYPE)((value<SLC_LOG_SP_MIN)? 0.0f: expf(value)));
        ...
        //{{IR distance distribution
        vtmpdsts.Reserve(nmaxdstvalsp1);
        vtmpdsts.AssignTo(optdstgapl);
        vtmpdsts.SetValueAt(0, 0.0f);
        ...
        if(( p = strstr(buffer.c_str(), patstrD)) != NULL ) {
            lineread = false;
            p += strlen( patstrD );
            ...
            vtmpdsts.SetValueAt(0, (float)ndsts);
            ...
            for( t = 1; t <= ndsts; t++ )
            {
                ...
                vtmpdsts.SetValueAt(t, (float)intval);
            }
        }//if patstrD
        //}}DD
        ...
        //set IR distance data
        for( r = 0; r < nmaxdstvalsp1; r++ )
            pm2dvec.SetFieldNext( pmv2DDDvalues + r, 0, (FPTYPE)vtmpdsts.GetValueAt(r));
        //
        if( SSSset ) {
            //first, set the address
...
//     char msgbuf[BUF_MAX];
    ...
    fprintf( fp, "%s %s%s", patstrDESC, desc.empty()? "": desc.c_str(), NL );
    fprintf( fp, "%s %s%s", patstrFILE, file.empty()? "": file.c_str(), NL );

    fprintf( fp, "%s", patstrCMD );   print_cmdline( &file_print, fp );
    fprintf( fp, "%s %d%s", patstrLEN, prolen, NL );
    fprintf( fp, "%s %d%s", patstrNOS, 0/*nodata*/, NL );
    fprintf( fp, "%s %.1f%s", patstrEFFNOS, (float)*(FPTYPE*)pm2dvec.GetField(pps2DENO,0), NL );
    ...
    for( t = 0; t < P_NSTATES; t++ ) {
        ...
        fprintf( fp, " %d", 
            (int)rintf( scale * expf((float)*(FPTYPE*)pm2dvec.GetField( ptr2DTrnPrbs + tt, 0 ))));
    }
...
//         fprintf( fp, "%s", NL);
        ...
        for( t = 0; t < P_NSTATES; t++ ) {
            ...
            fprintf( fp, " %d", 
                (int)rintf( scale * expf((float)*(FPTYPE*)pm2dvec.GetField( ptr2DTrnPrbs + tt, (m+1)*SZFPTYPE ))));
        }
...
//         fprintf( fp, " %d %d", 0, 0/*nodata*/);

        //{{HDP1
        fprintf( fp, "%s", NL);
        fprintf( fp, " %d %d", 0/*nodata*/, 1 );
        ...
        fprintf( fp, " %d", (int)*(INTYPE*)pm2dvec.GetField( pmv2DHDP1ind, m*SZINTYPE ));
        //}}
        ...
        z = (int)*(FPTYPE*)pm2dvec.GetField( pmv2DDDvalues, m*SZFPTYPE );
        if(0 < z) {
            fprintf( fp, "%s %s %d", NL, patstrD, z);
            for( t = 1; t <= z; t++ )
                fprintf(fp," %d",(int)rintf((float)*(FPTYPE*)pm2dvec.GetField(pmv2DDDvalues + t, m*SZFPTYPE )));
        }
        //}}
    ...
    fprintf( fp, "%s %6.4f %6.4f%s", patstrSPCOMP, 0.0f, 0.0f/*nodata*/, NL );
    fprintf( fp, "%s %6.4f %6.4f%s", patstrSPREFR, 0.0f, 0.0f/*nodata*/, NL );
...
//             pm2dvec.SetFieldNext( ptr2DTrnPrbsExp + tt, 0, (FPTYPE)((value<SLC_LOG_SP_MIN)? 0.0f: expf(value)));
        ...
        if(( p = strstr(pdata, patstrD)) != NULL ) {
            lineread = false;
            p += strlen( patstrD );
