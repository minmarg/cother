...
protected:
    ...
    void SetDbSize( size_t nentries, uint64_mt dbsize ) {
...
private:
    ...
    uint64_mt totprodbsize_;//total profile database size in positions (over all databases)
    size_t totndbentries_;//total number of database entries (over all databases)
    //}}
    //{{database attributes:
    uint64_mt prodbsize_;//profile database size in positions
