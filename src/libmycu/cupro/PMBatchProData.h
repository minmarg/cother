...
//         pmdata[ptr2DTrnPrbsExp+i] += SZFPTYPE * (prolen+1);
    ...
    for( i = 0; i < ptr2DNoDstsPerPos; i++ )
        pmdata[pmv2DDDvalues+i] += SZFPTYPE * prolen;
...
// -------------------------------------------------------------------------
// PMDataSkipNPros: move pointers to the buffers containing profile 
// data to the position obtained by skipping npros profiles
//
inline
...
//         pmdata[ptr2DTrnPrbsExp+i] += SZFPTYPE * (totlen+npros);
    ...
    for( i = 0; i < ptr2DNoDstsPerPos; i++ )
        pmdata[pmv2DDDvalues+i] += SZFPTYPE * totlen;
    for( i = 0; i < pmv2DNoSSSps; i++ )
...
//         szpmdata1[ptr2DTrnPrbsExp+i] = SZFPTYPE * (prolen+1);
    ...
    for( i = 0; i < ptr2DNoDstsPerPos; i++ )
        szpmdata1[pmv2DDDvalues+i] = SZFPTYPE * prolen;
    for( i = 0; i < pmv2DNoSSSps; i++ )
...
size_t PMBatchProData::GetPMDataSize1( size_t prolen )
{
    ...
    size += SZFPTYPE * ptr2DNoDstsPerPos * prolen;//[pmv2DDDvalues]
...
//         szpmdata1[ptr2DTrnPrbsExp+i] = SZFPTYPE * (totprolen+totprolen);
    ...
    for( i = 0; i < ptr2DNoDstsPerPos; i++ )
        szpmdata1[pmv2DDDvalues+i] = SZFPTYPE * totprolen;
    for( i = 0; i < pmv2DNoSSSps; i++ )
