    ...
    char** pmdata )
{
    ...
    const int maxndstvals = MAX_N_IR_DISTANCE_VALUES;
    extspsl::Pslvector ctxvec;//context vector
    extspsl::Pslvector dstvec;//IR distance vector
    extspsl::Pslvector uninfctxvec(pmv2DNoCVEls);//uninformative context vector
    extspsl::Pslvector emptydstvec(maxndstvals);//logically empty IR distance vector
    ...
    //set uninformative IR distance vector
    emptydstvec.AssignTo(optdstgapl);
    ...
    for( m = 0; m < prolen; m++ )
    {
        ...
        //{{IR distances
        //init distances to an empty record
        dstvec.SetVector(emptydstvec.GetVector(), maxndstvals);
        len = strlen(patstrD);
        if( strncmp(p,patstrD,len)) {
            sprintf( msgbuf, "Wrong profile format at pos %d: "
                            "No IR distance data.", m );
            throw MYRUNTIME_ERROR( preamb + msgbuf );
        }
        p += len;
        fp->incpos(len);
        ...
        fp->incpos(len);
        if( fp->eof())
           throw MYRUNTIME_ERROR(preamb+"Wrong profile format: No IR distance distribution size.");
        if( ptr2DNoDstsPerPos <= szdstd || szdstd < 0 ) {
           sprintf( msgbuf, "Wrong profile format at pos %d: "
                            "Invalid IR distance distribution size.", m );
           throw MYRUNTIME_ERROR( preamb + msgbuf );
        }
        if(szdstd) {
            //DD: IR distance values:
            len = sizeof(float) * szdstd;
            dstvec.SetVector((float*)p, szdstd);
            p += len;
            fp->incpos(len);
            if( fp->eof())
                throw MYRUNTIME_ERROR(preamb+"Wrong profile format: No IR distance data.");
        }
        //}}IR distances
        ...
        pm2dvec.SetFieldNext( pmv2DDDvalues, 0, (FPTYPE)szdstd);
        int di = 0;
        for(; di < szdstd; di++)
            pm2dvec.SetFieldNext( pmv2DDDvalues+1+di, 0, (FPTYPE)dstvec.GetValueAt(di));
        for(; di < maxndstvals; di++)
            pm2dvec.SetFieldNext( pmv2DDDvalues+1+di, 0, (FPTYPE)emptydstvec.GetValueAt(di));
        //
        if( SSSset ) {
            //first, set the address
