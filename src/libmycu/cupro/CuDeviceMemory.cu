...
// -------------------------------------------------------------------------
// CacheDDMS2Scores: cache precalculated distance match scores and the map 
// between distance distr. match scores and translated scores
// 
void CuDeviceMemory::CacheDDMS2Scores()
{
    MYMSG("CuDeviceMemory::CacheDDMS2Scores", 6);
    if( MOptions::GetDDMSWGT() <= 0.0f )
        return;
    CacheDDMS2ScoresDevice();
}
...
//&tmpszdp2diagblkprobscales, &tmpszdp2probscales,
    ...
    for( int i = 0; i < maxit; i++ ) {
        ...
        if(tmpszovlpos < leftoversize && 
           ...
           tmpmaxsizedbposs <= maxamountofleftover) 
        {
...
// -------------------------------------------------------------------------
// CacheDDMS2ScoresDevice: cache precalculated match scores the map between 
// the distribution match and translated scores
// 
void CuDeviceMemory::CacheDDMS2ScoresDevice()
{
    MYMSG("CuDeviceMemory::CacheDDMS2ScoresDevice", 4);
    const mystring preamb = "CuDeviceMemory::CacheDDMS2ScoresDevice: ";

    //SerializedDstMatchScoresCtor<CUBSM_TYPE> dms;
    SerializedDstMatchScoresCtor<DMS_TYPE,DMS_APPROXIMATE_SCORES> dms;

    //size to align the section of SSS scores:
    const size_t cszalnment = GetMemAlignment();
    size_t szscores = dms.GetSizeAlloc();

    if( deviceallocsize_ <= szscores + sz_heapsections_[0][ddsEndOfCVS2SMap])
    {
        char msgbuf[BUF_MAX];
        sprintf( msgbuf, "Insufficient amount of allocated device memory: "
                "%zu (ssss+cvs2s+ddms2s, %zu).", deviceallocsize_,
    ...
    if(szscores) {
        MYCUDACHECK( cudaMemcpy(d_eofcvs2sdata, dms.GetScores(), szscores, cudaMemcpyHostToDevice));
        MYCUDACHECKLAST;
    }
    ...
    for(int a = 0; a < nareas_; a++)
        sz_heapsections_[a][ddsEndOfDDMS2SMap] = sz_heapsections_[0][ddsEndOfCVS2SMap] + szscores;

    dmsattr_.ntotents_ = dms.GetNEntries();
    dmsattr_.szalloc_ = dms.GetSizeAlloc();
    dmsattr_.card_ = dms.GetCardinality();
}
...
void CuDeviceMemory::CacheHDPScoresDevice( const HDPscores& hdpscores )
{
    ...
    if( deviceallocsize_ <= szscores + sz_heapsections_[0][ddsEndOfDDMS2SMap])
    {
        char msgbuf[BUF_MAX];
//         sprintf( msgbuf, "Size of HDP scores exceeds allocated device memory size: %lu.", 
//                 deviceallocsize_ );
        sprintf( msgbuf, "Insufficient amount of allocated device memory: "
                "%zu (ssss+cvs2s+ddms2s+hdp1s, %zu).", deviceallocsize_,
                szscores + sz_heapsections_[0][ddsEndOfDDMS2SMap]);
        throw MYRUNTIME_ERROR( preamb + msgbuf );
    }

    char* d_eofddms2sdata = d_heap_ + sz_heapsections_[0][ddsEndOfDDMS2SMap];

    MYCUDACHECK( cudaMemcpy(d_eofddms2sdata, hdp1s.GetScores(), szscores, cudaMemcpyHostToDevice));
    MYCUDACHECKLAST;

    szscores = ALIGN_UP( szscores, cszalnment );

    for(int a = 0; a < nareas_; a++)
        sz_heapsections_[a][ddsEndOfHDPscores] = sz_heapsections_[0][ddsEndOfDDMS2SMap] + szscores;
    ...
    resDesc.res.linear.devPtr = d_eofddms2sdata;
