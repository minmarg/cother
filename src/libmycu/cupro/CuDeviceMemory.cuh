...
#include "libmycu/cupro/SerializedDstMatchScoresAttr.h"
...
public:
    ...
    enum TDevDataSections {
        ...
        ddsEndOfCVS2SMap,//end of the map between context vector scores and translated scores
        ddsEndOfDDMS2SMap,//end of the map between distance distr. match scores and translated scores
...
//ddsEndOfDP2BwdMtx,//end of phase 2 section of backward probability matrix
    ...
    SerializedDstMatchScoresAttr& GetDMSAttr() {return dmsattr_;}
...
private:
    ...
    SerializedDstMatchScoresAttr dmsattr_;//attributes of sereliazed distance match scores
...
void CuDeviceMemory::MsgAddressTable( int ano, const mystring preamb, const int level ) const
{
    MYMSGBEGl(level)
        ...
        MYMSGnonl((preamb + msgbuf).c_str(),3);
        sprintf(msgbuf, "        +%zu ddsEndOfDDMS2SMap", sz_heapsections_[ano][ddsEndOfDDMS2SMap]);
        MYMSGnonl((preamb + msgbuf).c_str(),3);
    ...
    ) const
{
    ...
    *maxsizedbposs = PMBatchProData::GetPMDataSizeUBTotal( maxdbposs );
    //*maxsizedbposs = SLC_MIN( *maxsizedbposs, maxamountfordbposs );//NOTE:maxdbposs unchanged
