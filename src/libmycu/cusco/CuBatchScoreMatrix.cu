...
// ddms2scores, serialized distance match scores;
// dmsattr, attributes of serialized distance match scores;
    ...
    CUBSM_TYPE* outmodscores )
{
    ...
    const float ddmswgt = MOptions::GetDDMSWGT();
    float hdp1swgt = MOptions::GetADJWGT(); 
    if( ssswgt > 0.0f )
        hdp1swgt *= ADJWGTDG;

    const int dstsegm = MOptions::GetDSTSEGM();
...
//         } else
        ...
        if( ddmswgt > 0.0f ) {
            //{{Scoring the match between distance distributions
            //                       STEP-----|        |-----LIN.INTERP.
            CalcSM_DDMS2S_SMEMUnroll2x_h2<DMS_STEP,DMS_LINEAR_INTERP_HALF,DMS_DSTBNDF>
                <<<nblcks,nthrds,dmsattr.szalloc_,streamproc>>>(
                    ddms2scores,
                    dmsattr,
                    ddmswgt,
                    dstsegm,
