...
my $mpicomms = MPI_Comm_size(MPI_COMM_WORLD);
my $nworkers = $mpicomms - 1;
...
if($nworkers < 1) {
    if($mpirank==0) {
        printf( STDERR GetDatetime()."No worker processes launched. Exiting...\n");
    }
    MPI_Finalize();
    exit( 0 );
}
...
if($#alnfiles < 0) {
    if($mpirank==0) {
        printf( STDERR GetDatetime()."No alignment files read. Exiting...\n");
    }
    MPI_Finalize();
    exit( 0 );
}
...
my $nperprocess = int(($#alnfiles+1 + $nworkers-1)/$nworkers);

if($mpirank==0) {
    print( STDERR GetDatetime()."Processing alignments... ($nperprocess/process)\n");
}
##computation follows
if($mpirank) {
    MPIProcUpdateFreqs($mpirank, $nworkers, \@alnfiles, \$noalns, \@nopss);
}
else {
    ##get results from the processes
    my $dummy;
    my @rresults = MPI_Gather($dummy, 0, MPI_COMM_WORLD);
    foreach my $rr (@rresults) {
        next unless $rr;
        $tret = $$rr{RET};
        unless($tret) {
            print( STDERR GetDatetime()."ERROR: A process failed.\n");
            next;
        }
        ##infuse process data
        $noalns += ${$$rr{ALNS}};
        ...
        print( STDERR GetDatetime()."  results from a process obtained.\n");
    }
}
##}}
...
sub MPIProcUpdateFreqs
{
    my $myrid = shift;
    my $mynwrks = shift;##total number of workers
    ...
    my (%thres);
    $ret = 1;
    for($i = $myrid-1; $i <= $#$rafiles; $i+=$mynwrks) {
        $f = $$rafiles[$i];
    ...
    MPI_Gather(\%thres, 0, MPI_COMM_WORLD);
}
